...
extension PasteboardType {
    ...
    nonisolated static let supportedTypes: [PasteboardType] = [
        // Image types first to prioritize image detection over fileURL
    ...
    nonisolated static let publicURL = NSPasteboard.PasteboardType("public.url")

    nonisolated static let jpeg = NSPasteboard.PasteboardType("public.jpeg")
    nonisolated static let heic = NSPasteboard.PasteboardType("public.heic")
    nonisolated static let heif = NSPasteboard.PasteboardType("public.heif")
    nonisolated static let gif = NSPasteboard.PasteboardType("com.compuserve.gif")
    nonisolated static let webp = NSPasteboard.PasteboardType("org.webmproject.webp")
    nonisolated static let bmp = NSPasteboard.PasteboardType("com.microsoft.bmp")
    nonisolated static let flatRTFD = NSPasteboard.PasteboardType("com.apple.flat-rtfd")
    nonisolated static let microsoftObjectLink = NSPasteboard.PasteboardType(rawValue: "com.microsoft.ObjectLink")
    nonisolated static let microsoftLinkSource = NSPasteboard.PasteboardType(rawValue: "com.microsoft.Link-Source")
    nonisolated static let universalClipboard = NSPasteboard.PasteboardType(rawValue: "com.apple.is-remote-clipboard")
    nonisolated static let autoGenerated = NSPasteboard.PasteboardType(rawValue: "org.nspasteboard.AutoGeneratedType")
    nonisolated static let concealed = NSPasteboard.PasteboardType(rawValue: "org.nspasteboard.ConcealedType")
    nonisolated static let source = NSPasteboard.PasteboardType(rawValue: "org.nspasteboard.source")
    nonisolated static let transient = NSPasteboard.PasteboardType(rawValue: "org.nspasteboard.TransientType")
    nonisolated static let modified = NSPasteboard.PasteboardType(rawValue: "x.nspasteboard.ModifiedType")
    nonisolated static let fromMaccy = NSPasteboard.PasteboardType(rawValue: "org.p0deje.Maccy")
    nonisolated static let linkPresentationMetadata = NSPasteboard.PasteboardType(rawValue: "com.apple.linkpresentation.metadata")
    nonisolated static let customWebKitPasteboardData = NSPasteboard.PasteboardType(rawValue: "com.apple.WebKit.custom-pasteboard-data")
    nonisolated static let customChromiumWebData = NSPasteboard.PasteboardType(rawValue: "org.chromium.web-custom-data")
    nonisolated static let chromiumSourceUrl = NSPasteboard.PasteboardType(rawValue: "org.chromium.source-url")
    nonisolated static let chromiumSourceToken = NSPasteboard.PasteboardType(rawValue: "org.chromium.internal.source-rfh-token")
    nonisolated static let notesRichText = NSPasteboard.PasteboardType(rawValue: "com.apple.notes.richtext")
    nonisolated static let deckUnsupported = NSPasteboard.PasteboardType(rawValue: "com.deck.unsupported")
    
    nonisolated func isImage() -> Bool {
    ...
    nonisolated func isText() -> Bool {
        !isImage() && !isFile()
    }
    
    nonisolated func isFile() -> Bool {
        self == .fileURL
    }
}
...
final class ClipboardItem: Identifiable, Equatable {
    struct PasteboardSnapshot: Sendable {
        struct Item: Sendable {
            ...
            let stringByType: [String: String]

            nonisolated var pasteboardTypes: [PasteboardType] {
                types.map { PasteboardType(rawValue: $0) }
            }

            nonisolated func data(forType type: PasteboardType) -> Data? {
                dataByType[type.rawValue]
            }

            nonisolated func string(forType type: PasteboardType) -> String? {
                stringByType[type.rawValue]
            }
        }
        ...
        nonisolated var firstItem: Item? {
            items.first
        }
    }

    struct ParsedPayload: Sendable {
        let pasteboardTypeRaw: String
        let data: Data
    ...
    nonisolated private static func decodeStringFromData(_ data: Data?) -> String? {
    ...
    private struct PasteboardItemReader {
        nonisolated let types: [PasteboardType]
        nonisolated(unsafe) let dataForType: (PasteboardType) -> Data?
        nonisolated(unsafe) let stringForType: (PasteboardType) -> String?

        nonisolated init(
            types: [PasteboardType],
            dataForType: @escaping (PasteboardType) -> Data?,
            stringForType: @escaping (PasteboardType) -> String?
        ) {
            self.types = types
    ...
    private nonisolated static let utf16FallbackTypes: [PasteboardType] = [
    ...
    private nonisolated static let genericTextFallbackExcludedTypes: Set<PasteboardType> = [
    ...
    nonisolated private static func resolveTextFallbackPayload(
        from item: NSPasteboardItem,
        filteredTypes: Set<PasteboardType>
    ) -> TextFallbackResolution {
        let reader = PasteboardItemReader(
            types: item.types,
            dataForType: { item.data(forType: $0) },
            stringForType: { item.string(forType: $0) }
        )
        return resolveTextFallbackPayload(from: reader, filteredTypes: filteredTypes)
    }

    nonisolated private static func resolveTextFallbackPayload(
        from item: PasteboardSnapshot.Item,
        filteredTypes: Set<PasteboardType>
    ) -> TextFallbackResolution {
        let reader = PasteboardItemReader(
            types: item.pasteboardTypes,
            dataForType: { item.data(forType: $0) },
            stringForType: { item.string(forType: $0) }
        )
        return resolveTextFallbackPayload(from: reader, filteredTypes: filteredTypes)
    ...
    nonisolated private static func resolveTextFallbackPayload(
        from item: PasteboardItemReader,
        filteredTypes: Set<PasteboardType>
    ) -> TextFallbackResolution {
    ...
    nonisolated private static func resolveHTMLTextPayload(from item: PasteboardItemReader) -> TextFallbackResolution {
    ...
    nonisolated private static func resolveUTF16PlainText(from item: PasteboardItemReader) -> TextFallbackPayload? {
    ...
    nonisolated private static func resolveGenericTextPayload(
        from item: PasteboardItemReader,
        filteredTypes: Set<PasteboardType>
    ) -> TextFallbackPayload? {
    ...
    nonisolated private static func makePlainTextPayload(_ text: String, mode: String) -> TextFallbackPayload? {
    ...
    nonisolated private static func makePreviewData(from attributed: NSAttributedString, type: PasteboardType) -> Data? {
    ...
    nonisolated private static func containsFigmaMarker(in html: String) -> Bool {
    ...
    nonisolated private static func normalizeFilePath(_ raw: String) -> String {
    ...
    nonisolated static func searchTextForFilePaths(_ paths: [String]) -> String {
    ...
    convenience init(parsedPayload payload: ParsedPayload, sourceAnchor: SourceAnchor? = nil) {
        self.init(
            pasteboardType: PasteboardType(rawValue: payload.pasteboardTypeRaw),
            data: payload.data,
    ...
    func parsedPayloadForTransfer() -> ParsedPayload {
        ParsedPayload(
            pasteboardTypeRaw: pasteboardType.rawValue,
            data: data,
            previewData: previewData,
            timestamp: timestamp,
            appPath: appPath,
    ...
    nonisolated static func parseSnapshotPayload(_ snapshot: PasteboardSnapshot) -> ParsedPayload? {
        guard let item = snapshot.firstItem else { return nil }

        let filteredTypes = Self.filteredPasteboardTypes(from: item.pasteboardTypes)

        guard let type = PasteboardType.supportedTypes.first(where: { filteredTypes.contains($0) }) else {
            let rawTypes = item.types.joined(separator: ",")
            switch Self.resolveTextFallbackPayload(from: item, filteredTypes: filteredTypes) {
            case .payload(let payload):
                log.debug("Creating item with text fallback [\(payload.mode)] types: [\(rawTypes)]")
                return ParsedPayload(
                    pasteboardTypeRaw: payload.pasteboardType.rawValue,
                    data: payload.data,
                    previewData: payload.previewData,
            ...
            if let imageData = snapshot.fallbackImageData {
                let previewData = Self.generatePreviewThumbnailData(from: imageData)
                return ParsedPayload(
                    pasteboardTypeRaw: PasteboardType.png.rawValue,
                    data: imageData,
                    previewData: previewData,
                    timestamp: snapshot.timestamp,
                    appPath: snapshot.appPath,
        ...
        if type == .publicURL {
            ...
            filePaths = Self.filePaths(from: snapshot, item: item)
            guard !filePaths.isEmpty else { return nil }
            ...
            content = item.data(forType: type)
            if content == nil, type.isImage(), let imageData = snapshot.fallbackImageData {
                resolvedType = .png
                content = imageData
            }
        }
        ...
        return ParsedPayload(
            pasteboardTypeRaw: resolvedType.rawValue,
            data: content,
            previewData: previewData,
            timestamp: snapshot.timestamp,
            appPath: snapshot.appPath,
    ...
    convenience init?(snapshot: PasteboardSnapshot) {
        guard let payload = Self.parseSnapshotPayload(snapshot) else { return nil }
        self.init(parsedPayload: payload)
    }
    ...
    nonisolated private static func filePaths(from snapshot: PasteboardSnapshot, item: PasteboardSnapshot.Item) -> [String] {
        if !snapshot.fileURLPaths.isEmpty {
            return snapshot.fileURLPaths
        }
    ...
    nonisolated private static func resolveFilePathToken(_ token: String) -> String? {
        let trimmed = token.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return nil }
        return normalizeFilePath(trimmed)
    }

    private nonisolated static let microsoftSourcePrefix = "com.microsoft.ole.source."

    nonisolated private static func filteredPasteboardTypes(from types: [PasteboardType]) -> Set<PasteboardType> {
    ...
    nonisolated private static func normalizedPlainText(_ text: String) -> String {
        let stripped = text
    ...
    nonisolated private static func imageDataFromAttributedString(_ attributedString: NSAttributedString) -> Data? {
    ...
    nonisolated private static func detectItemType(
        pasteboardType: PasteboardType,
        searchText: String,
        contentLength: Int,
        filePaths: [String]
        ...
        case .fileURL:
            if Self.filePathsAreImages(filePaths) {
        ...
        case .rtf, .rtfd, .flatRTFD:
            ...
            let sample: String
            let maxAnalysisLength = 8_000
            if contentLength > maxAnalysisLength,
               let end = rawText.index(rawText.startIndex, offsetBy: maxAnalysisLength, limitedBy: rawText.endIndex) {
    ...
    nonisolated private static func filePathsAreImages(_ paths: [String]) -> Bool {
        guard !paths.isEmpty else { return false }
        ...
        return paths.allSatisfy { path in
            let resolvedPath = Self.normalizeFilePath(path)
            let ext = (resolvedPath as NSString).pathExtension.lowercased()
            if knownImageExtensions.contains(ext) { return true }
            if !ext.isEmpty, let type = UTType(filenameExtension: ext), type.conforms(to: .image) {
                return true
            }
            return false
    ...
    static func generatePreviewThumbnailDataFromImageFile(at url: URL, maxSize: CGFloat = 200) -> Data? {
        guard FileManager.default.fileExists(atPath: url.path) else { return nil }
        guard let cg = cgImageThumbnailFromFileURL(url, maxPixelSize: Int(maxSize)) else { return nil }
        let bitmapRep = NSBitmapImageRep(cgImage: cg)
        return bitmapRep.representation(using: .png, properties: [:])
    }

    /// iOS 同步等场景：落盘文件是否值得预生成 `preview_data`（缩小列表查询对「路径解码」的依赖）。
    static func shouldPregenerateListPreviewForFilesystemImage(at url: URL) -> Bool {
    ...
    nonisolated private static func downsampledCGImage(fromFileURL url: URL, maxPixelSize: Int) -> CGImage? {
        cgImageThumbnailFromFileURL(url, maxPixelSize: maxPixelSize)
    }
